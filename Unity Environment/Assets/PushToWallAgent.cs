...
public class PushToWallAgent : Agent {
	...
	public override void AgentReset()
	{
		...
		//transform.position = behindBlock.position;
		transform.localEulerAngles = new Vector3(0.0f, Random.Range(0.0f, 359.0f));
		//configuration = Random.Range(0, 5);
		configuration = 3;
	...
	private void OnCollisionStay(Collision collision)
	{
		...
		{
			if (transform.position.y <= collision.gameObject.transform.position.y && transform.position.z < collision.gameObject.transform.position.z && oldPosition.z < transform.position.z)
			{
				AddReward(0.001f);
			}
